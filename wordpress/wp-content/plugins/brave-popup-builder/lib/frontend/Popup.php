    ...
    class BravePop_Popup {
        ...
        function __construct($popupID=null, $popupType='popup', $forceLoad=false, $forceStep=false, $customContent=false ) {
               ...
               if($this->popupID && $this->popupData && $popupType === 'popup') {
                  if($this->userTypeMatch && $this->refererMatch && $this->languageMatch && $this->countryMatch && $this->hasCartItems && !in_array(false, $this->cartFilterMatch) && $this->purchaseMatch && $this->notpurchaseMatch && $this->learnDashMatch && $this->EDDMatch) {
      ...
      function popup_render_element($element, $stepIndex, $index, $device, $goalElementIDs, $ajaxRender ){
         $elements = '';
         switch ($element->type) {
            case 'text':
            ...
            case 'social':
               if (class_exists('BravePop_Element_Social')) {
                  $elmCalss = new BravePop_Element_Social($element, $this->popupID, $stepIndex, $index, $device, in_array($element->id, $goalElementIDs ));
                  $elements .= $elmCalss->render();
                  $elmStyle = $elmCalss->render_css();
                  $this->elementStyles[$element->id] = $elmStyle;
               }
      ...
      public function popup_renderElements($stepSettings, $stepIndex, $device, $ajaxRender) { 
         if(!$stepSettings){ return ''; }
               ...
               if(isset($stepSettings->content)){
                  foreach ($stepSettings->content as $index => $element) {
                     ...
                     if(isset($element->type) && $element->type !== 'group' && empty($element->group) && (!isset($element->hidden) || $element->hidden === false)){
                        $elements .= $this->popup_render_element($element, $stepIndex, $index, $device, $goalElementIDs, $ajaxRender );
                     }
                        ...
                        if (class_exists('BravePop_Element_Group')) {
                           ...
                           if(isset($element->elements)){
                              foreach ($element->elements as $chid => $childElmID) {
                                 ...
                                 if($childElement){
                                    $childrenHTML .= $this->popup_render_element($childElement, $stepIndex, $index, $device, $goalElementIDs, $ajaxRender );
                                 }
                              }
      ...
      public function popup_renderPopup($stepSettings, $stepIndex, $device, $ajaxRender) { 
         if(!$stepSettings){ return ''; }
                  ...
                  '<div class="brave_popup__step__inner">
                     <div class="brave_popupSections__wrap">
                        <div class="brave_popupMargin__wrap">
                           '.$closeButtonAbove .'
                           <div class="brave_popup__step__popup">'.$this->popup_renderElements($stepSettings, $stepIndex, $device, $ajaxRender).'</div>
                           '.$closeButtonBelow .'
                        </div>
                     </div>
                  </div>';
      ...
      public function popup_render_content($ajaxRender=false){
         $popupContentHTML = '';
         if($this->popupData->steps){
            foreach ($this->popupData->steps as $key => $step) {
               $exitAnim = '';
               ...
               if(isset($step->mobile)){
                  ...
                  $popupContentHTML .= '<div class="brave_popup__step brave_popup__step__mobile '.$this->popup_renderCSSClasses($step->mobile).'" data-width="'.esc_attr($mobileStepWidth).'" data-height="'.esc_attr($mobileStepHeight).'" data-open="false" style="z-index:'.(99999 - $key).'" '.$noMobileContentData.' '.$mobileExitAnimType.' '.$mobileExitAnimDuration.' '.$mobilePopupLayout.' '.$mobilePopupPosition.'>'.$this->popup_renderPopup($step->mobile, $key, 'mobile', $ajaxRender).''.$this->popup_renderOverlay($step->mobile, $key).'</div>';
               } 
