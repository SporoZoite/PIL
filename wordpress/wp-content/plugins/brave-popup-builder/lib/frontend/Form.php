...
if ( ! class_exists( 'BravePop_Element_Form' ) ) {
   ...
   class BravePop_Element_Form {
         ...
         <script>
            document.addEventListener("DOMContentLoaded", function(event) {
               ...
               brave_popup_formData['<?php print_r(esc_attr($this->data->id)); ?>'] = {
                  ...
                  freemailAllow: <?php print_r(json_encode(!empty($this->data->formData->settings->action->allowfreemail) ? true : false));?>,
               }
               <?php if($this->hasDate){  
      ...
      public function render_css() { 
         ...
         $progressColorStyle .= $progressBorder ?  '#brave_popup_'.$this->popupID.'__step__'.$this->stepIndex.' #brave_element-'.$this->data->id.' .braveformBolt{ '. $progressBorder.' margin-right: calc('.($this->totalSteps > 2 ? round(100 / ($this->totalSteps - 1)) : 50).'% - 24px);}' : '';
      ...
      protected function renderDate($field){
         ...
         $fieldHTML = '<div id="brave_form_field'.$field->id.'" class="brave_form_field brave_form_field--date" '.$startDate.' '.$endDate.'>';
            $fieldHTML .= $label ? '<label class="braveform_label">'.$field->label.$requiredStar.'</label>' : '';
            $fieldHTML .= '<div class="brave_form_field_error"></div>';
            $fieldHTML .= $dateType !== 'dropdown' ? '<input type="text" placeholder="'.esc_attr($placeholder).'" name="'.$fieldName.'" autoComplete="off" />':'';
            ...
            if($dateType === 'dropdown'){
               ...
               for ($y=1930; $y < (date("Y")+1); $y++) { $yearsSelection[] = $y;} $yearsSelection = array_reverse($yearsSelection);
                  ...
                  $fieldHTML .= '<select id="brave_form_field'.$fieldName.'-date" name="'.$fieldName.'">';
                  ...
                  $fieldHTML .= '<select id="brave_form_field'.$fieldName.'-month" name="'.$fieldName.'">';
                  ...
                  if($dateFormatNoYear === false){
                     $fieldHTML .= '<select id="brave_form_field'.$fieldName.'-year" name="'.$fieldName.'">';
